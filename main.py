...
def run_script():
    ...
    state = State.Start
    yield say(
        'Привет! Я буду задавать вопросы на умножение, а ты должен отвечать. Всё просто!\n '
        'Готов учить таблицу умножения?',
        suggest('Да', 'Готов', 'Давай', 'Конечно', 'Начнем'))
    while True:
        if request.has_lemmas('Помощь', 'Что ты умеешь?'):
            yield say('Я умею задавать вопросы, напрмер "2 на 2", а ты должен ответить верно. Я проверю!\n'
                      'Готов учить таблицу умножения?', suggest('Начнём'))
